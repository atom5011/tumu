...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField, Header("今回のゲームで生成される干支の種類")]
    ...
    IEnumerator Start()
    {
        ...
        //干支データのリストが生成されていなければ
        if(GameData.instance.etoDataList.Count == 0)
        {
            //干支データのリストを作成。この処理が終了するまで、次の処理へは行かないようにする
            yield return StartCoroutine(GameData.instance.IniEtoDataList());
        }

        //今回のゲームに登場する干支をランダムで選択。この処理が終了するまで、次の処理にいかないようにする
